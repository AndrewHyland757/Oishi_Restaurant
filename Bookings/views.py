...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        # If the user is not logged in, the guest booking form will be used which contains extra fields
        if request.method == 'POST':
...
def view_bookings(request):
    """
    Gets the reservations from the logged in user based on their email or user name (incase the superuser has not an email)
